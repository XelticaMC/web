<template>
  ...
  <p>
    ...
    このクライアントが、サーバーという目には見えない遠くにあるコンピューターに接続するようになっており、
    同じサーバーに接続しているクライアント同士は共に同じワールドでゲームを楽しむことができます。
  </p>
  <p>
    サーバーとクライアントは、お互いに共通の言葉で会話をすることで、マルチプレイを実現します。これをプロトコルと呼びます。
  ...
  <p>
    しかし、Floodgateを使用することで、統合版クライアントを検知した場合、認証機構をスキップすることができます。統合版は（アプリストアを介するため）買わないと入手できないので、
    認証機構は必要ないですからね。
  </p>
</template>
